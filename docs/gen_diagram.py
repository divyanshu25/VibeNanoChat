...
arrow_v(
    ...
    y + ch,
    y + ch + GAP,
    "Trainer(sft_training, checkpoint_path, depth_override, …)",
)
...
arrow_v(
    ...
    y + ch,
    y + ch + GAP,
    "trainer.train()  →  torch.set_float32_matmul_precision('high')",
)
...
dy += pill(
    ...
    dy,
    cw,
    "🔁  Training Loop  —  for epoch in range(num_epochs):  for step in range(max_steps):",
)
...
trigger_items = [
    (
        ...
        [
            "est. val loss over ~10.5M tokens",
            "BPB on eval split",
            "sample_from_model() → log text",
        ],
    ),
    (
        "CORE BENCHMARK  (every core_evals_after steps)",
        [
            "MMLU · HellaSwag · ARC · LAMBADA",
            "likelihood-ranking, centered score",
            "core_score = mean(centered)",
        ],
    ),
    (
        "CHATCORE  (end of each epoch)",
        [
            "GSM8K · HumanEval · ARC · MMLU",
    ...
    (
        ...
        [
            "model + optimizer state_dicts",
            "epoch, step, global_step",
            "master_process only (rank 0)",
        ],
    ),
]
        ...
        [
            (
                ...
                [
                    (
                        "bullets",
                        ["MMLU · HellaSwag · ARC-Easy/Challenge · LAMBADA · SQuAD"],
                    ),
                ],
            ),
            (
...
out = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "training_pipeline.excalidraw"
)
