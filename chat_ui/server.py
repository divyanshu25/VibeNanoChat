...
def generate_response(prompt: str, max_new_tokens: int = 256, temperature: float = 0.8, top_k: int = 50):
...
async def list_checkpoints():
    ...
    try:
        ...
        if not checkpoint_dir.exists():
            return JSONResponse(content={"checkpoints": [], "error": f"Directory not found: {CHECKPOINT_DIR}"})
        
        checkpoints = sorted([
            str(f.relative_to(checkpoint_dir))
            for f in checkpoint_dir.glob("**/*.pt")
        ])
        
        return JSONResponse(content={
            "checkpoints": checkpoints,
            "current": os.path.basename(current_checkpoint) if current_checkpoint else None
        })
...
async def chat(request: ChatRequest):
    ...
    if model is None:
        raise HTTPException(status_code=400, detail="No checkpoint loaded. Please load a checkpoint first.")
