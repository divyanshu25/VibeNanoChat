...
make interactive-gen CHECKPOINT=logs/step_19531.pt
```
...
│   │   ├── ddp.py                # Distributed training entry point
...
│   │       ├── batch_scaling.py  # Automatic batch/LR/WD scaling
...
│   │   └── multiplex_dataloader.py  # Multi-dataset mixing for SFT
│   └── eval_tasks/               # Evaluation benchmarks
│       ├── core/                 # Base model evals (MMLU, HellaSwag, etc.)
│       ├── chat_core/            # Chat model evals (GSM8K, HumanEval, etc.)
│       └── training/             # Training-time evaluation
├── data/
│   ├── fineweb_edu/              # Dataset preparation scripts
│   ├── openwebtext/              # OpenWebText preparation
│   └── task_mixture/             # Task mixture datasets
├── scripts/
│   ├── chat.py                   # Interactive chat interface
│   ├── plot_scaling_laws.py      # Scaling law visualization
│   ├── plot_isoflop_curve.py     # Isoflop curve plotting
│   ├── verify_flops_formula.py   # Verify C = 6ND formula
│   └── verify_scaling_constant.py
├── debug_tools/
│   ├── interactive_generate.py   # Simple text generation
│   └── demo_kvcache_speedup.py   # KV cache benchmarking
├── chat_ui/                      # Web-based chat interface
├── tests/                        # Unit and integration tests
├── docs/                         # Deep technical documentation
├── resources/eval_bundle/        # Evaluation benchmark data
└── Makefile                      # Training commands
```

**Core components:**

- **GPT-2 architecture** (`src/gpt_2/`): Standard transformer with causal attention, RoPE positional embeddings, pre-norm LayerNorm, sliding window attention
- **Muon optimizer** (`src/gpt_2/muon.py`): Momentum-based optimizer with Newton-Schulz orthogonalization for adaptive learning
- **Depth parameterization** (`src/gpt_2/config.py`): Single `DEPTH` parameter controls model width and depth
- **Automatic batch scaling** (`src/gpt_2/training_utilities/batch_scaling.py`): Learning rate, weight decay, and batch size auto-scale with model size using Power Lines law (B ∝ D^0.383)
- **Data loading** (`src/dataloaders/`): Memory-mapped Parquet files for zero-copy I/O, BOS-aligned document packing
- **Evaluation** (`src/eval_tasks/`): 35+ benchmarks integrated into training loop (MMLU, HellaSwag, PIQA, ARC, etc.)
- **KV cache** (`src/gpt_2/kv_cache.py`): 3-10× faster generation with cached attention keys/values
...
**Example dimensions:**
- `DEPTH=8`  → 8 layers, 512 dims, 8 heads (~77M params)
- `DEPTH=12` → 12 layers, 768 dims, 12 heads (~154M params)
- `DEPTH=20` → 20 layers, 1280 dims, 20 heads (~522M params)
...
3. **Scalable**: Start with the 10BT sample for quick experiments. Scale up to 100BT or the full 1.3T tokens when you have more compute.
4. **BOS-aligned packing**: Documents are packed with BOS tokens, preserving document boundaries for better learning dynamics.
...
# Or run directly
uv run python debug_tools/interactive_generate.py logs/step_19531.pt
```
...
uv run python scripts/chat.py --checkpoint logs/checkpoint.pt
...
The interactive generation script allows you to adjust:
- Temperature: Controls randomness (0.1-2.0, default 0.8)
- Repetition penalty: Reduces repetition (1.0-1.5, default 1.2)
...
make run-scaling-law  # Run full scaling law experiments
make run-depth-sweep  # Run depth sweep experiments
make gpu-status       # Check GPU utilization, memory, temperature
make kill-gpu         # Kill zombie processes holding GPU memory
make interactive-gen  # Interactive text generation
make chat-server      # Start web UI for chat
...
| [README_MULTIPLEX.md](docs/README_MULTIPLEX.md) | Multi-dataset mixing for supervised fine-tuning |
| [README_CHATCORE_EVALUATOR.md](docs/README_CHATCORE_EVALUATOR.md) | Chat model evaluation (GSM8K, HumanEval, ARC) |
| [SLIDING_WINDOW_ATTENTION.md](docs/SLIDING_WINDOW_ATTENTION.md) | Memory-efficient attention with sliding windows |
