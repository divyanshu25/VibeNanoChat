...
from gpt_2.training_utilities import (setup_dataloaders, setup_evaluators,
                                      setup_hyperparameters, setup_logging,
                                      setup_model, setup_wandb)
from gpt_2.utils import (get_lr_multiplier, get_scaling_params,
...
class Trainer:
    ...
    def __init__(
        ...
        # Set up synchronize function for accurate timing measurements
        # CUDA operations are asynchronous, so we need to sync for wall-clock timing
        self.synchronize = (
            torch.cuda.synchronize if device.startswith("cuda") else lambda: None
        )
        ...
        # Store scaled hyperparameters from batch scaling (pretrain only)
        self.batch_lr_scale = hyperparams["batch_lr_scale"]
        self.weight_decay_scaled = hyperparams["weight_decay_scaled"]
        self.embedding_lr = hyperparams["embedding_lr"]
        self.unembedding_lr = hyperparams["unembedding_lr"]
        self.matrix_lr = hyperparams["matrix_lr"]
        self.scalar_lr = hyperparams["scalar_lr"]

        # Setup remaining components (dataloaders, evaluators, optimizer, wandb)
        self._setup_data_and_evaluators()
        self._setup_optimizer_and_checkpoint()
        self._setup_wandb()

    def _setup_data_and_evaluators(self):
        """
        Setup training dataloaders and evaluation components.

        Training dataloader yields infinite stream of batches (wraps around dataset).
        Evaluators are used periodically to estimate val loss and run benchmarks.
        """
        # Setup dataloaders
        self.train_dataloader, eval_dataloader = setup_dataloaders(
            sft_training=self.sft_training,
            config=self.config,
            ddp_world_size=self.ddp_world_size,
    ...
    def _setup_optimizer_and_checkpoint(self):
        ...
        2. Weight decay scaling: Use T_epoch framework (λ ∝ √(B/B_ref) · (D_ref/D)) to maintain
           consistent regularization dynamics across different model sizes and batch sizes.
        ...
        Note: Batch scaling is already applied in setup_hyperparameters(), so we just use
        the pre-computed scaled values here.
        """
        ...
        # Note: weight_decay_scaled is already stored as instance variable from setup_hyperparameters()
        # and is used for dynamic scheduling during training (see get_muon_weight_decay())
    ...
    def _load_checkpoint_and_set_counters(self):
        ...
        """
        # Detect checkpoint type and determine scenario
        is_pretrain_ckpt = "pretrain_checkpoints" in self.checkpoint_path
        is_sft_ckpt = "sft_checkpoints" in self.checkpoint_path

        # Map to one of three scenarios
        scenario = None
        if self.sft_training and is_pretrain_ckpt:
            scenario = "rollover"  # Transfer learning
        elif not self.sft_training and is_pretrain_ckpt:
            scenario = "resume_pretrain"
        elif self.sft_training and is_sft_ckpt:
            scenario = "resume_sft"
        else:
            raise ValueError(
        ...
        if scenario == "rollover":
            ...
            if self.master_process:
                mode = "pretraining" if scenario == "resume_pretrain" else "SFT"
                print(
    ...
    def _print_training_info(
        self, total_steps, total_tokens, total_flops, num_params, tokens_params_ratio
    ):
        """Print training configuration and statistics."""
        print(f"\n{SEPARATOR}")
        ...
        print(f"🔢 Scaling parameters: {num_params:,}")
        ...
        self._print_eval_schedule(total_steps)
        print(f"{SEPARATOR}\n")

    def _print_eval_schedule(self, total_steps):
        """Print evaluation schedule information."""
        if self.run_evals:
            num_val_evals = (
                total_steps // self.run_evals_after if self.run_evals_after > 0 else 0
            )
            val_type = (
        ...
        if self.run_core_evals:
            ...
            core_type = (
                "manual" if self.core_eval_interval_override is not None else "adaptive"
            )
            print(
                f"📊 Core benchmark evals: every {self.run_core_evals_after} steps → ~{num_core_evals} total ({core_type})"
            )

    def _run_validation_and_sampling(self, step, global_step, flops_so_far):
        """Run validation loss evaluation and optionally sample from model."""
        ...
        if self.config.enable_sampling and self.master_process:
            ...
            for i, context in enumerate(self.sample_contexts, 1):
                print(f"Context {i}/{len(self.sample_contexts)}: {context[:50]}...")
    ...
    def _save_checkpoint_if_needed(self, step, epoch, global_step, val_loss):
    ...
    def _log_training_metrics(
        ...
        self._log_learning_rates(log_dict)
        wandb.log(log_dict)
    ...
    def _log_learning_rates(self, log_dict):
        ...
        # Log AdamW group learning rates (includes per-layer scalars)
        if len(adamw_groups) >= 6:
            # With per-layer scalars: resid_lambdas, x0_lambdas, lm_head, embedding, value_embeds, other
            log_dict["lr/resid_lambdas"] = adamw_groups[0]["lr"]
            log_dict["lr/x0_lambdas"] = adamw_groups[1]["lr"]
            log_dict["lr/unembedding"] = adamw_groups[2]["lr"]
            log_dict["lr/embedding"] = adamw_groups[3]["lr"]
            log_dict["lr/value_embeds"] = adamw_groups[4]["lr"]
            log_dict["lr/scalar"] = adamw_groups[5]["lr"]
        ...
        if len(muon_groups) > 0:
            ...
            if "weight_decay" in muon_groups[0]:
                log_dict["muon/weight_decay"] = muon_groups[0]["weight_decay"]

    def _update_learning_rates(
        self, global_step, lrm, muon_momentum, muon_weight_decay
    ):
    ...
    def _compute_tokens_per_second(self, num_active_tokens, start_time, end_time):
        """Calculate throughput (tokens/second) for the current step."""
        if self.sft_training:
            # SFT: only count tokens we actually trained on (y >= 0)
            return num_active_tokens.item() / (end_time - start_time)
        else:
    ...
    def _print_dataloader_stats(self):
        """Print and log dataloader packing statistics."""
        if not hasattr(self.train_dataloader, "get_stats"):
            return

        try:
            stats = self.train_dataloader.get_stats()

            # Aggregate stats across all GPU ranks
            if self.ddp:
            ...
            if self.master_process:
                rank_label = (
                    f"All {self.ddp_world_size} Ranks" if self.ddp else "Single Process"
                )
                print(f"\n{SEPARATOR}")
                print(f"📦 DATALOADER PACKING STATS (Final - {rank_label})")
                print(SEPARATOR)
                print(f"   Total tokens received: {stats['total_tokens']:,}")
                print(f"   Processed tokens (packed): {stats['processed_tokens']:,}")
                print(
                ...
                wandb.log(
                    {
                        ...
                        "dataloader/cropped_tokens_pct": stats["cropped_tokens_pct"],
    ...
    def train(self):
        ...
        if self.master_process:
            ...
            num_params = get_scaling_params(self.raw_model)
            tokens_params_ratio = total_tokens / num_params
            self._print_training_info(
                total_steps, total_tokens, total_flops, num_params, tokens_params_ratio
            )
        ...
        for epoch in range(self.start_epoch, self.num_epochs):
            ...
            for step in range(epoch_start_step, self.max_steps):
                ...
                # Without this, GPU work from previous iteration may still be running
                self.synchronize()
                ...
                self._update_learning_rates(
                    global_step, lrm, muon_momentum, muon_weight_decay
                )
                ...
                tokens_per_second = self._compute_tokens_per_second(
                    num_active_tokens, start_time, end_time
                )
                ...
                if self.run_evals and should_run_validation:
                    val_loss = self._run_validation_and_sampling(
                        step, global_step, flops_so_far
                    )
                ...
                should_run_core_benchmarks = (
                    global_step % self.run_core_evals_after == 0
                    or global_step == total_steps - 1
                )
                if self.run_core_evals and should_run_core_benchmarks:
                    self.core_evaluator.evaluate_all_tasks(
                        step=step, global_step=global_step, flops_so_far=flops_so_far
                    )
                ...
                self._save_checkpoint_if_needed(step, epoch, global_step, val_loss)
            ...
            if self.run_chatcore_evals:
                ...
                if self.master_process:
                    ...
                    for task_name, results in chatcore_results.items():
                        print(
                            f"   {task_name}: {results['accuracy']:.2%} ({results['correct']}/{results['total']})"
                        )
                    print(f"{SEPARATOR}\n")
