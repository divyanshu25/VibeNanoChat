...
def get_scaling_params(model):
    """
    Get the number of parameters used for scaling law calculations.

    Following nanochat convention: transformer_matrices + lm_head gives the
    ...
    Returns:
        int: Number of scaling parameters (transformer matrices + lm_head)
    """
    # Get detailed parameter counts from model
    param_counts = model.num_scaling_params()

    # Scaling parameters = transformer weight matrices + output projection head
    # These are the parameters that scale predictably with model size
    # Embeddings are excluded because they scale with vocabulary, not model depth
    scaling_params = param_counts["transformer_matrices"] + param_counts["lm_head"]

    return scaling_params


def has_value_embedding(layer_idx, n_layer):
    """
    Determine if a layer should have value embeddings.

    Uses alternating pattern based on last layer's parity, with the last layer always included.
    This ensures value embeddings are distributed across the model while guaranteeing the
    final layer has them for optimal output representations.
    ...
    Returns:
        bool: True if this layer should have value embeddings

    Examples:
        For 4 layers (last=3, odd): layers 1, 3 have VE
        For 6 layers (last=5, odd): layers 1, 3, 5 have VE
        For 8 layers (last=7, odd): layers 1, 3, 5, 7 have VE
    """
...
def save_checkpoint(
    ...
    Checkpoint filename format:
        Depth mode: step{global_step}_d{depth}_{mode}.pt
        Non-depth mode: step{global_step}_{mode}.pt

    Checkpoint subdirectory:
        Depth mode: {checkpoint_dir}/d{depth}/
        Non-depth mode: {checkpoint_dir}/default/

    Examples:
        step10000_d14_pretrain.pt (depth mode)
        step5000_d18_sft.pt (depth mode)
        step10000_pretrain.pt (non-depth mode)
    ...
    Args:
        ...
        depth: Model depth (None or <= 0 for non-depth mode, positive int for depth mode)
    """
    total_steps = max_steps * num_epochs
    ...
    # Create subdirectory based on architecture mode
    # Depth mode: use d{depth} subdirectory (e.g., d12, d18)
    # Non-depth mode: use 'default' subdirectory
    if depth is not None and depth > 0:
        depth_dir = os.path.join(checkpoint_dir, f"d{depth}")
    else:
        depth_dir = os.path.join(checkpoint_dir, "default")
    ...
    checkpoint_path = os.path.join(depth_dir, checkpoint_filename)
