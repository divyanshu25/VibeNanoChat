...
class GPTConfig:
    ...
    n_kv_head: int | None = (
        None  # Number of KV heads for GQA (None = auto-set to n_head for MHA)
    )
    n_embed: int = (
        1280  # Embedding dimension (hidden size) = depth(20) * aspect_ratio(64)
    )
    logit_softcap: float = (
        15.0  # Smoothly cap logits to range [-softcap, softcap] using tanh
    )

    # ========================================================================
    # Sliding Window Attention (nanochat-style)
    # ========================================================================
    # Pattern string tiled across layers. Final layer always gets full context.
    # Characters: L=long (full context), S=short (half context)
    # Examples: "L" = all full context, "SL" = alternating, "SSSL" = three short, one long
    window_pattern: str = "SSSL"  # Nanochat default: 3 short windows, 1 long window
    ...
    total_batch_size: int = -1  # Total tokens per gradient update (2^19)
    ...
    # 1. target_flops (if > 0): calculate iterations to reach target FLOPs
    #    This will compute the param_data_ratio from FLOPs
    # 2. target_param_data_ratio (if > 0): calculate iterations from data:param ratio (Chinchilla=20)
    ...
    # ========================================================================
    # NOTE: Training steps (num_iterations) are automatically calculated from
    # target_flops or target_param_data_ratio (see "Training horizon" section above).
    # The batch_scaling module computes iterations dynamically for pretraining.
    ...
    chat_core_hf_cache_dir: str = (
        "<YOURPATH>/sft_cache"  # HuggingFace cache directory for datasets
    )
    ...
    def __post_init__(self):
        ...
        if self.depth > 0:
            ...
            # If n_kv_head was not explicitly set, default to MHA (n_kv_head = n_head)
            if self.n_kv_head is None:
                self.n_kv_head = self.n_head

            # Validate that n_head is divisible by n_kv_head
            if self.n_head % self.n_kv_head != 0:
                raise ValueError(
                    f"n_head ({self.n_head}) must be divisible by n_kv_head ({self.n_kv_head}). "
                    f"Valid GQA ratios: 1:1 (MHA), 2:1, 4:1, 8:1 (MQA), etc."
                )
