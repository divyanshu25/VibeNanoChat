...
def reliability_guard(maximum_memory_bytes: Optional[int] = None):
    ...
    if platform.uname().system != "Darwin" and maximum_memory_bytes is not None:
        ...
        # Also skip if maximum_memory_bytes is None (no limit)
        import resource

        try:
            # Try to set memory limits, but don't fail if we can't raise them
            resource.setrlimit(
                resource.RLIMIT_AS, (maximum_memory_bytes, maximum_memory_bytes)
            )
        except (ValueError, OSError):
            pass  # Can't set this limit, continue anyway
        ...
        try:
            # Get current stack limit and only lower it if needed
            soft, hard = resource.getrlimit(resource.RLIMIT_STACK)
            if soft == resource.RLIM_INFINITY or soft > maximum_memory_bytes:
                resource.setrlimit(resource.RLIMIT_STACK, (maximum_memory_bytes, hard))
        except (ValueError, OSError):
            pass  # Can't set this limit, continue anyway
