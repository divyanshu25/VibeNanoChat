...
class TestKVCacheBasicOperations:
    ...
    def test_reset(self, device, dtype):
        ...
        k = torch.randn(1, 4, 10, 64, device=device, dtype=dtype)
        v = torch.randn(1, 4, 10, 64, device=device, dtype=dtype)
    ...
    def test_lazy_initialization(self, device, dtype):
        ...
        k = torch.randn(1, 4, 5, 64, device=device, dtype=dtype)
        v = torch.randn(1, 4, 5, 64, device=device, dtype=dtype)
        kv_cache.insert_kv(0, k, v)
    ...
    def test_insert_and_retrieve(self, device, dtype):
        ...
        k1 = torch.randn(batch_size, num_heads, 5, head_dim, device=device, dtype=dtype)
        v1 = torch.randn(batch_size, num_heads, 5, head_dim, device=device, dtype=dtype)
        ...
        k2 = torch.randn(batch_size, num_heads, 3, head_dim, device=device, dtype=dtype)
        v2 = torch.randn(batch_size, num_heads, 3, head_dim, device=device, dtype=dtype)
    ...
    def test_dynamic_growth(self, device, dtype):
        ...
        k = torch.randn(1, 4, 20, 64, device=device, dtype=dtype)  # 20 > 16
        v = torch.randn(1, 4, 20, 64, device=device, dtype=dtype)
    ...
    def test_prefill_from_single_to_batch(self, device, dtype):
        ...
        k = torch.randn(1, 4, 10, 32, device=device, dtype=dtype)
        v = torch.randn(1, 4, 10, 32, device=device, dtype=dtype)
    ...
    def test_prefill_validation(self, device, dtype):
...
class TestGenerationEquivalence:
    ...
    @pytest.fixture
    def test_model(self, device, dtype):
