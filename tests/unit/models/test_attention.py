...
class TestAttentionForward:
    ...
    def test_attention_forward_basic(self, device, dtype):
        """Test basic attention forward pass."""
        B, n_head, T, head_dim = 2, 4, 8, 64

        q = torch.randn(B, n_head, T, head_dim, device=device, dtype=dtype)
        k = torch.randn(B, n_head, T, head_dim, device=device, dtype=dtype)
        v = torch.randn(B, n_head, T, head_dim, device=device, dtype=dtype)
    ...
    def test_attention_forward_gqa(self, device, dtype):
        """Test attention forward with GQA (grouped query attention)."""
        B, n_head, n_kv_head, T, head_dim = 2, 8, 2, 16, 64

        q = torch.randn(B, n_head, T, head_dim, device=device, dtype=dtype)
        k = torch.randn(B, n_kv_head, T, head_dim, device=device, dtype=dtype)
        v = torch.randn(B, n_kv_head, T, head_dim, device=device, dtype=dtype)
    ...
    def test_attention_forward_non_causal(self, device, dtype):
...
class TestCausalSelfAttention:
    ...
    def test_forward_basic(self, small_config, device, dtype):
        """Test basic forward pass without RoPE or KV cache."""
        attn = CausalSelfAttention(small_config, layer_idx=0).to(device).to(dtype)
    ...
    def test_forward_with_rope(self, small_config, device, dtype):
        ...
        cos, sin = precompute_rotary_embeddings(
            seq_len=T, head_dim=head_dim, device=device, dtype=dtype
        )
    ...
    def test_forward_gqa(self, gqa_config, device, dtype):
        """Test forward pass with GQA."""
        attn = CausalSelfAttention(gqa_config, layer_idx=0).to(device).to(dtype)
    ...
    def test_different_sequence_lengths(self, small_config, device, dtype):
    ...
    def test_qk_normalization(self, small_config, device, dtype):
        ...
        x = torch.randn(B, T, C, device=device, dtype=dtype) * 10.0
    ...
    def test_causal_masking(self, small_config, device, dtype):
        ...
        x = torch.zeros(B, T, C, device=device, dtype=dtype)
    ...
    def test_batch_size_independence(self, small_config, device, dtype):
        ...
        x_single = torch.randn(1, T, C, device=device, dtype=dtype)
        ...
        for i in range(4):
            assert torch.allclose(output_single[0], output_batched[i], atol=1e-2)

    def test_gradient_flow(self, small_config, device, dtype):
        ...
        # Note: value_embed_gate might not have gradients if value_embed=None (not used)
        for name, param in attn.named_parameters():
            if "value_embed_gate" in name:
                # value_embed_gate only gets gradients if value_embed is provided
                continue
            assert param.grad is not None, f"No gradient for {name}"
            assert not torch.isnan(param.grad).any(), f"NaN gradient in {name}"

    def test_determinism(self, small_config, device, dtype):
    ...
    def test_parameter_count(self, small_config):
        ...
        # value_embed_gate: only exists for layers with value embeddings (alternating pattern)
        expected_value_embed_gate_params = 0
        if attn.value_embed_gate is not None:
            # value_embed_gate: (n_kv_head, 32) - Linear layer with input 32, output n_kv_head
            expected_value_embed_gate_params = 32 * attn.n_kv_head

        total_params = sum(p.numel() for p in attn.parameters())
        expected_total = (
            expected_c_attn_params
            + expected_c_proj_params
            + expected_value_embed_gate_params
        )
...
class TestRoPEIntegration:
    ...
    def test_rope_shape_compatibility(self, device, dtype):
        ...
        attn = CausalSelfAttention(config, layer_idx=0).to(device).to(dtype)
    ...
    def test_rope_position_encoding(self, device, dtype):
        ...
        assert not torch.allclose(output_with_rope, output_without_rope, atol=1e-2)
...
class TestGQABehavior:
    ...
    def test_gqa_different_head_counts(self, device, dtype):
...
class TestNumericalStability:
    ...
    def test_large_sequence_length(self, device, dtype):
    ...
    def test_extreme_input_values(self, device, dtype):
        ...
        test_cases = [
            torch.randn(B, T, C, device=device, dtype=dtype) * 100,  # Large values
            torch.randn(B, T, C, device=device, dtype=dtype) * 0.001,  # Small values
            torch.zeros(B, T, C, device=device, dtype=dtype),  # All zeros
        ]
    ...
    def test_mixed_precision_compatibility(self, device, dtype):
        """Test that attention works with bfloat16."""
        config = GPTConfig()
        config.n_embed = 256
        config.n_head = 4
        config.n_kv_head = 4  # MHA
...
class TestEdgeCases:
    ...
    def test_single_token_sequence(self, device, dtype):
    ...
    def test_minimum_config(self, device, dtype):
