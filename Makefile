...
# Sample SFT: make ddp-train NGPUS=2 MODE=sft CHATCORE_EVALS=true DEPTH=12 CHECKPOINT=<YOUR_PATH>/divgoyal/nanogpt/pretrain_checkpoints/model_checkpoint_global37953_pretraining.pt
...
chat-server: ## Start the chat web UI server on port 8003
	...
	@echo "📁 Checkpoint directory: <YOUR_PATH>/divgoyal/nanogpt/midtrain_checkpoints"
...
interactive-gen: ## Run interactive text generation. Usage: make interactive-gen [CHECKPOINT=/path/to/checkpoint.pt]
	@CHECKPOINT=$${CHECKPOINT:-<YOUR_PATH>/divgoyal/nanogpt/pretrain_checkpoints/step3092_depth12_pretrain.pt}; \
