...
def calculate_training_flops(
    steps: np.ndarray, batch_size: int, flops_per_token: float
) -> np.ndarray:
    """
...
def fit_scaling_law(flops: np.ndarray, losses: np.ndarray) -> Tuple[np.ndarray, Dict]:
    ...
    try:
        # Fit the curve
        popt, pcov = curve_fit(
            power_law, flops, losses, p0=p0, bounds=bounds, maxfev=50000, method="trf"
        )
...
def plot_scaling_laws(data_list: List[Dict], save_path: str = None):
    ...
    for i, data in enumerate(data_list):
        ...
        final_flops = calculate_training_flops(
            np.array([final_step]), data["batch_size"], data["flops_per_token"]
        )[0]
    ...
    if popt is not None:
        ...
        flops_smooth = np.logspace(
            np.log10(all_flops.min()), np.log10(all_flops.max()), 200
        )
    ...
    ax1.set_title(
        "Scaling Laws: Validation BPB vs Training FLOPs\n(Log-Log Scale)",
        fontsize=14,
        fontweight="bold",
    )
    ax1.set_xscale("log")
